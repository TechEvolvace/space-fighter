...
public class Game1 : Game
{
    ...
    {
        _tieTexture = Content.Load<Texture2D>("imgs/TieFighter");
        _explosionTexture = Content.Load<Texture2D>("imgs/explosion");

        _enemySpawner = new EnemySpawner(SCREEN_WIDTH, SCREEN_HEIGHT);
        ...
        _playerFalcon = new Falcon(_falconTexture, _playerLaserTexture, startPos, GraphicsDevice);
    ...
    {
        // 1. Update the Falcon (movement + firing)
        _playerFalcon.Update(gameTime, keyboardState, mouseState);

        // 2. Spawn and update enemies
        _enemySpawner.Update(gameTime, _tieTexture, _explosionTexture, _playerLaserTexture);

        // 3. Update Tie Fighters + collisions
        foreach (var tie in _enemySpawner.TieFighters)
        {
            if (tie.IsAlive || tie.IsExploding())
            {
                // IMPORTANT: Update() now handles UpdateLasers(gameTime)
                tie.Update(gameTime, _playerFalcon.Position, _playerFalcon.Texture, 0.25f);

                Rectangle falconRect = new Rectangle(
                    (int)_playerFalcon.Position.X - _playerFalcon.Texture.Width / 2,
                    (int)_playerFalcon.Position.Y - _playerFalcon.Texture.Height / 2,
                    _playerFalcon.Texture.Width,
                    _playerFalcon.Texture.Height
                );

                Rectangle tieRect = new Rectangle(
                    (int)tie.Position.X,
                    (int)tie.Position.Y,
                    (int)(tie.Texture.Width * 0.05f),
                    (int)(tie.Texture.Height * 0.05f)
                );

                // Falcon hits TieFighter
                if (tie.IsAlive && !tie.IsExploding() && falconRect.Intersects(tieRect))
                {
                    _playerFalcon.TakeDamage(25);
                    tie.Explode();
                }

                // Falcon hits enemy lasers
                foreach (var enemyLaser in tie.EnemyLasers.ToList())
                {
                    Rectangle laserRect = new Rectangle(
                    (int)enemyLaser.Position.X - 1,
                    (int)enemyLaser.Position.Y,
                    2, 20   // matches original long laser shape
                );

                    if (falconRect.Intersects(laserRect))
                    {
                        _playerFalcon.TakeDamage(10);
                        tie.EnemyLasers.Remove(enemyLaser);
                    }
                }

                // Falcon lasers hit TieFighter
                for (int i = _playerFalcon.Lasers.Count - 1; i >= 0; i--)
                {
                    var laser = _playerFalcon.Lasers[i];
                    Rectangle laserRect = new Rectangle(
                        (int)laser.Position.X - 2,
                        (int)laser.Position.Y - 2,
                        4, 4
                    );

                    if (tie.IsAlive && !tie.IsExploding() && tieRect.Intersects(laserRect))
                    {
                        tie.Explode();
                        _playerFalcon.Lasers.Remove(laser);
                        AddScore(100);
        ...
        _hudDisplay.UpdateHealth(_playerFalcon.Health);
        _hudDisplay.UpdateEnemiesDefeated(_enemiesDefeated);

        // 5. Win/Loss conditions
        if (_playerFalcon.Health <= 0)
        {
            _playerVictorious = false;
    ...
    protected override void Draw(GameTime gameTime)
    {
        ...
        switch (_currentGameState)
        {
            case GameState.Playing:
            case GameState.Paused:
                // Draw the Falcon and its lasers
                _playerFalcon.Draw(_spriteBatch);

                // Draw enemies
                foreach (var tie in _enemySpawner.TieFighters)
                {
                    if (tie.IsAlive || tie.IsExploding())
                        tie.Draw(_spriteBatch); // This includes explosions and enemy lasers
                }
