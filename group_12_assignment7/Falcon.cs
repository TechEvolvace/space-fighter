...
public class Falcon
{
    ...
    public Falcon(Texture2D falconTexture, Texture2D laserTexture, Vector2 startPosition)
    {
        this.Texture = falconTexture;
    ...
    private void HandleMovement(KeyboardState kState, float delta)
    {
        ...
        // Keep the Falcon on screen
        int width = Texture.Width / 2;
        int height = Texture.Height / 2;

        Position = Vector2.Clamp(Position,
                                 new Vector2(width, height),
                                 new Vector2(1280 - width, 720 - height)); // Assuming 1280x720 screen
    }
